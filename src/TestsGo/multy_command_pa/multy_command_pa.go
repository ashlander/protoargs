...
func PrepareOptions(program string) (*flag.FlagSet, *Config) {
    ...
    config.ArgCOMMAND = StringValue{``, false}
...
func ParseExt(program string, args []string, description string, allow_incomplete bool) (*Config, error) {
    ...
    if !allow_incomplete && flags.NArg() < 1 {
        errArgCOMMAND := errors.New(`Required positional 'COMMAND' is missing`)
        fmt.Println(errArgCOMMAND)
        fmt.Println(Usage(program, description))
        return config, errArgCOMMAND
    }
    errArgCOMMAND := config.ArgCOMMAND.Set(flags.Arg(0))
    if !allow_incomplete && errArgCOMMAND != nil {
        fmt.Println(errArgCOMMAND)
        fmt.Println(Usage(program, description))
        return config, errArgCOMMAND
    }
