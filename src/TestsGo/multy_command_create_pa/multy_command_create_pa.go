package multy_command_create_pa
...
func PrepareOptions(program string) (*flag.FlagSet, *Config) {
    ...
    config.Argsize = Uint64Value{0, false}
    config.ArgPATH = StringValue{``, false}
    ...
    flags.Var(&config.Argsize, `s`, `Size of the file {OPTIONAL,type:uint64,default:0}`)
    flags.Var(&config.Argsize, `size`, `Size of the file {OPTIONAL,type:uint64,default:0}`)
...
func Usage(program string, description string) string {
    ...
    var limit uint32 = 80
    block := "\n" + `usage: ` + program + ` [-h|--help] [-s|--size size] PATH`
  ...
  -s, --size size        Size of the file {OPTIONAL,type:uint64,default:0})`
    usage += splitUsage(block, limit)
    usage += "\n" 
...
func ParseExt(program string, args []string, description string, allow_incomplete bool) (*Config, error) {
    ...
    if !allow_incomplete && flags.NArg() < 1 {
        errArgPATH := errors.New(`Required positional 'PATH' is missing`)
        fmt.Println(errArgPATH)
        fmt.Println(Usage(program, description))
        return config, errArgPATH
    }
    errArgPATH := config.ArgPATH.Set(flags.Arg(0))
    if !allow_incomplete && errArgPATH != nil {
        fmt.Println(errArgPATH)
        fmt.Println(Usage(program, description))
        return config, errArgPATH
    }
