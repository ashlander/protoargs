...
{
    # Common Variables
    multy_command_create_PROTOARGS_USAGE=""
...
function multy_command_create_usage() #(program, description)
{
    ...
    multy_command_create_PROTOARGS_USAGE="$(cat << PROTOARGS_EOM
...
#
# returns `multy_command_create_PROTOARGS_ARGS` Resulting set of args
function multy_command_create_remove_args_tail() #(keep, args)
{
    multy_command_create_PROTOARGS_ARGS=()
    local keep=$1
    shift # past number
    local pos=0
    while [[ "$pos" -lt "$keep" ]]; do
        multy_command_create_PROTOARGS_ARGS+=("$1")
        shift
        pos=$((pos + 1))
    done
}
...
#
# returns `multy_command_create_PROTOARGS_ARGS` Resulting set of args
function multy_command_create_keep_args_tail()
{
    local erase=$1
    shift # past number
    local pos=0
