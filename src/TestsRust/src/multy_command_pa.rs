...
pub fn prepare_options_ext<'a>(command: clap::Command<'a>) -> clap::Command<'a> {
    ...
    return command
        ...
        .arg(clap::Arg::new(r#"COMMAND"#)
           .help(r#"Command (create, copy) {REQUIRED,type:string}"#)
           .required(true)
           .index(1)
...
pub fn parse_matches(matches: &clap::ArgMatches, allow_incomplete: bool) -> Result<Config, String> {
    ...
    let has_command = matches.is_present("COMMAND");
    if !allow_incomplete && !has_command {
        return Err("Required 'COMMAND' is missing".to_string());
    }
    config.command = matches.value_of("COMMAND").map(|o| o.parse::<String>().ok()).flatten();
    if has_command && !config.has_command() {
        let err = format!("Type 'String' conversion failed for 'COMMAND' with value '{}'", matches.value_of("COMMAND").unwrap());
        return Err(err);
    }
